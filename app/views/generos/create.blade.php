...
@section('content')
      ...
      {{ Form::open(array('method' => 'POST', 'route' => array('dashboard.generos.store'))) }}
        ...
        <div class="well">
          <div id="newGenreContainer" style="height: 30px; width: 360px; margin-bottom:5px;">
            <button id="newGenreTitle" class="btn btn-block" type="button">Insertar Lenguage</button>
            <div id="newGenreComponent" class="input-append">
              <input id="txtGen" style="width: 240px" type="text" placeholder="Xénero">
          ...
          {{ Form::label('lang', trans('app.genres.lang').":") . AMG::displayErr($errors, 'nombre') }}
  ...
  <script>
    $(document).ready(function(){
      ...
      $('#newGenreContainer').bind({
        mouseenter: function(){ 
          $('#newGenreTitle').fadeOut(function() { $('#newGenreComponent').fadeIn() }); },
        mouseleave: function(){ 
          $('#newGenreComponent').fadeOut(function() { $('#newGenreTitle').fadeIn() }); }
      });
