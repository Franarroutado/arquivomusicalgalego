...
@section('content')
  ...
  <div class="btn-group">
      ...
      <a href="{{ route('dashboard.generos.create') }}" class="btn btn-primary"><i class="icon-plus"></i> @lang('button.new_entity', ['entity' => trans('button.genre')])</a>
  </div>
</div>
<div class="well">
    {{ Form::open(['route' => 'dashboard.generos.index', 'method' => 'GET', 'id' => 'searchForm', 'class' => 'navbar-search .pull-right']) }}
          ...
          @if (count($generos) > 0)
            @foreach ($generos as $genero)
              <tr>
                ...
                <td>
                    <a href="{{ route('dashboard.generos.edit', $genero->id) }}"><i class="icon-pencil"></i></a>
                    <a href="#myModal" class="deleteLink" data-id="{{ $genero->id }}" role="button" data-toggle="modal"><i class="icon-remove"></i></a>
                </td>
              </tr>
            @endforeach
          @else
