          ...
          @if (count($centros) > 0)
            @foreach ($centros as $centro)
              <tr>
                <td>{{ $centro->nombre }}</td> 
                <td>
                    <a href="{{ route('dashboard.centros.edit', $centro->id) }}"><i class="icon-pencil"></i></a>
                    <a href="#myModal" class="deleteLink" data-id="{{ $centro->id }}" role="button" data-toggle="modal"><i class="icon-remove"></i></a>
                </td>
              </tr>
            @endforeach
          @else
